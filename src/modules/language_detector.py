...
class LanguageDetector:
    ...
    def __init__(
        ...
        self.model_path = (
            Path(model_path) if model_path else config.MOD1_CLASSIFIER_PATH
        )
        self.vectorizer_path = (
            Path(vectorizer_path) if vectorizer_path else config.MOD1_VECTORIZER_PATH
        )
        self.fasttext_model_path = (
            Path(fasttext_model_path)
            if fasttext_model_path
            else config.ARTIFACTS_DIR / "lid.176.ftz"
        )
        ...
        if not self.fasttext_model_path.exists():
            raise FileNotFoundError(
                f"FastText model file not found: {self.fasttext_model_path}"
            )
