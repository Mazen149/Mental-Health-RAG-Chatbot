...
def router_exact_match(example, pred, trace=None, pred_name=None, pred_trace=None):
    ...
    return (
        1.0 if example.route.strip().lower() in str(pred_route).strip().lower() else 0.0
    )
...
class StandaloneQueryJudge(dspy.Signature):
    ...
    score = dspy.OutputField(
        desc="A float between 0.0 and 1.0. Output ONLY the number."
    )
...
def grounded_metric(example, pred, trace=None, pred_name=None, pred_trace=None):
    ...
    try:
        ...
        sentences = (
            answer_str.count(".") + answer_str.count("!") + answer_str.count("?")
        )
