...
# ---------------------------------------------------------------------------
from src.prompts.dspy_training_data import (  # noqa: E402
    get_router_dataset,
...
from src.prompts.dspy_evaluators import (  # noqa: E402
    router_exact_match,
    condenser_metric,
    grounded_metric,
    conversation_metric,
...
def diagnose(train_mean: float, test_mean: float) -> str:
    ...
    if test_mean >= 0.70:
        return "[OK] GOOD FIT -- High test performance, small gap (gap={:.2f})".format(
            gap
        )
    return "[..] MODERATE -- Acceptable gap but test score could improve (gap={:.2f})".format(
        gap
    )
...
def generate_charts(results: Dict[str, Dict[str, EvalResult]], output_dir: Path):
    ...
    base_train = [
        results[m].get("base_train", EvalResult("", "", "")).mean for m in module_names
    ]
    base_test = [
        results[m].get("base_test", EvalResult("", "", "")).mean for m in module_names
    ]
    ...
    ax.set_title(
        "DSPy Optimized Modules - Train vs Test Performance",
        fontsize=14,
        fontweight="bold",
    )
    ax.set_xticks(x)
    ...
    ax.set_ylabel("Mean Metric Score", fontsize=12, fontweight="bold")
    ax.set_title(
        "Optimized vs Base Model - Test Set Performance", fontsize=14, fontweight="bold"
    )
    ax.set_xticks(x)
    ax.set_xticklabels(module_names, rotation=20, ha="right", fontsize=10)
    ax.set_ylim(0, 1.15)
    ...
    bars = ax.barh(
        module_names, gaps, color=colors, edgecolor="white", linewidth=0.8, height=0.5
    )

    for bar, gap_val in zip(bars, gaps):
        w = bar.get_width()
        ax.text(
            w + 0.01 if w >= 0 else w - 0.06,
    ...
    ax.set_xlabel(
        "Train - Test Gap (positive = potential overfit)",
        fontsize=11,
        fontweight="bold",
    )
    ax.set_title(
        "Generalization Gap Analysis (Optimized Modules)",
        fontsize=14,
        fontweight="bold",
    )
    ...
    ax.plot(
        angles, opt_test_vals, "o-", linewidth=2, color="#FF9800", label="Optimized"
    )
    ...
    ax.set_ylim(0, 1.0)
    ax.set_title(
        "Test Set Performance - Radar View", fontsize=14, fontweight="bold", y=1.08
    )
    ...
    fig.suptitle(
        "Per-Sample Score Distributions - Train vs Test (Optimized)",
        fontsize=13,
        fontweight="bold",
    )
    fig.tight_layout()
    ...
    matrix = np.array([opt_train, opt_test, base_train, base_test, gaps])
...
def main():
    ...
    for spec in MODULE_SPECS:
        ...
        print(
            f"  Dataset: {len(full_dataset)} total -> {len(train_set)} train / {len(test_set)} test"
        )
        ...
        print(
            f"  >> Evaluating optimized module on TRAIN set ({len(train_set)} samples)..."
        )
        train_scores = evaluate_module(opt_module, train_set, spec.metric_fn, lm)
        module_results["optimized_train"] = EvalResult(
            spec.name, "optimized", "train", train_scores
        )
        print(f"    Mean train score: {module_results['optimized_train'].mean:.4f}")

        print(
            f"  >> Evaluating optimized module on TEST set ({len(test_set)} samples)..."
        )
        test_scores = evaluate_module(opt_module, test_set, spec.metric_fn, lm)
        module_results["optimized_test"] = EvalResult(
            spec.name, "optimized", "test", test_scores
        )
        ...
        try:
            ...
            print("  >> Evaluating BASE (unoptimized) module on TRAIN set...")
            base_train_scores = evaluate_module(
                base_module, train_set, spec.metric_fn, lm
            )
            module_results["base_train"] = EvalResult(
                spec.name, "base", "train", base_train_scores
            )
            print(f"    Mean base train: {module_results['base_train'].mean:.4f}")

            print("  >> Evaluating BASE (unoptimized) module on TEST set...")
            base_test_scores = evaluate_module(
                base_module, test_set, spec.metric_fn, lm
            )
            module_results["base_test"] = EvalResult(
                spec.name, "base", "test", base_test_scores
            )
            print(f"    Mean base test:  {module_results['base_test'].mean:.4f}")
        except Exception as e:
            print(f"  [WARN] Could not evaluate base module: {e}")
            module_results["base_train"] = EvalResult(
                spec.name, "base", "train", [0.0] * len(train_set)
            )
            module_results["base_test"] = EvalResult(
                spec.name, "base", "test", [0.0] * len(test_set)
            )
        ...
        if gain > 0:
            ...
            print("  [GAIN=] No measurable optimization gain on test set")
    ...
    print("=" * 70)
    print(
        f"{'Module':<25} {'Opt Train':>10} {'Opt Test':>10} {'Gap':>8} {'Base Test':>10} {'Gain':>8}  Diagnosis"
    )
    print("-" * 110)
    ...
    for name, res in all_results.items():
        ...
        print(
            f"{name:<25} {ot:>10.4f} {oe:>10.4f} {gap:>+8.4f} {bt:>10.4f} {gain:>+8.4f}  {diag}"
        )
