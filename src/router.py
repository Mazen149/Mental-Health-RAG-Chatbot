...
def translate_to_english(text: str, client=None) -> str:
    ...
    try:
        ...
        if _translator_pipeline is None:
            ...
            safe_print(
                "--> [Translator Setup] Initializing local multilingual translation model..."
            )
...
async def route_query(query: str, rag_instance, history: list = None) -> dict:
    ...
    if detect_medicine_query(query):
        safe_print(
            "--> [Router Guardrail] Matched medicine query keyword. Bypassing RAG and returning medical disclaimer."
        )
    ...
    if GREETING_REGEX.match(cleaned) and len(cleaned.split()) <= 2:
        ...
        if language in GREETING_REGEX_BY_LANG and GREETING_REGEX_BY_LANG[
            language
        ].match(cleaned):
        ...
        if matched_language:
            ...
            safe_print(
                f"--> [Router] Override language to {language} based on greeting pattern match."
            )
        return {
    ...
    if GOODBYE_REGEX.match(cleaned) and len(cleaned.split()) <= 3:
        ...
        matched_language = None
        if language in GOODBYE_REGEX_BY_LANG and GOODBYE_REGEX_BY_LANG[language].match(
            cleaned
        ):
            matched_language = language
        ...
        if matched_language:
            language = matched_language
            safe_print(
                f"--> [Router] Override language to {language} based on goodbye pattern match."
            )
        return {
    ...
    if GRATITUDE_REGEX.match(cleaned) and len(cleaned.split()) <= 3:
        ...
        matched_language = None
        if language in GRATITUDE_REGEX_BY_LANG and GRATITUDE_REGEX_BY_LANG[
            language
        ].match(cleaned):
            matched_language = language
        ...
        if matched_language:
            language = matched_language
            safe_print(
                f"--> [Router] Override language to {language} based on gratitude pattern match."
            )
        return {
    ...
    if intent == "crisis":
        ...
        safe_print(
            "--> [Router] Classified intent as CRISIS. Routing to localized safety template."
        )
        ...
        safe_print(
            "--> [Router] Routing to full Mental Health grounding RAG pipeline..."
        )
        ...
        safe_print(
            f"--> [Router] Routing to conversational LLM ({intent}) via Layer 2 fallback."
        )
...
async def preload_models():
    ...
    async def load_translator():
        try:
            ...
            if ENABLE_TRANSLATION:
                ...
                safe_print(
                    "--> Translator preloading skipped (ENABLE_TRANSLATION=False)."
                )
        except Exception as e:
    ...
    await asyncio.gather(
        load_detector(), load_emotion(), load_intent(), load_translator()
    )
