...
def mock_classifier():
    ...
    with patch(
        "src.modules.emotion_classifier.AutoModelForSequenceClassification.from_pretrained"
    ) as _mock_base, patch(
        "src.modules.emotion_classifier.PeftModel.from_pretrained"
    ) as mock_peft, patch(
        "src.modules.emotion_classifier.AutoTokenizer.from_pretrained"
    ) as mock_tokenizer, patch(
        "src.modules.emotion_classifier.Path.exists", return_value=True
    ):
