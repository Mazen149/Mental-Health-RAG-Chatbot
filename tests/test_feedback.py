...
class TestFeedback(unittest.TestCase):
    def setUp(self):
        ...
        for name, module in list(sys.modules.items()):
            if module is not None and (name == 'src.app' or name == 'app') and hasattr(module, '_is_authenticated'):
    ...
    def test_chat_with_message_payload(self, mock_route, mock_rag):
        ...
        response = self.client.post(
            "/chat",
            json={"message": "hello message field"}
        )
        
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertEqual(data["answer"], "This is another mock answer.")
        mock_route.assert_called_once_with("hello message field", mock_rag, history=None)
    ...
    def test_feedback_submission_success(self):
        ...
        self.assertEqual(response_up.json(), {"status": "ok", "message": "Feedback saved successfully."})
        ...
        self.assertEqual(response_down.json(), {"status": "ok", "message": "Feedback saved successfully."})

        # Query the database directly to verify persistence
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT vote, user_message, bot_response FROM feedback ORDER BY id ASC")
    ...
    def test_feedback_unauthenticated_uses_guest_user(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT id, username FROM users WHERE username = ?", ("guest",))
            guest_user = cursor.fetchone()
            self.assertIsNotNone(guest_user)
            guest_id = guest_user[0]

            cursor.execute("SELECT user_id, vote, user_message FROM feedback WHERE user_message = ?", ("guest query",))
