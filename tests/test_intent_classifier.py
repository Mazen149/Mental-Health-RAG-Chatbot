...
def test_intent_classifier_embedding_match():
    ...
    with patch.object(classifier, "_embedding_classifier") as mock_embed:
        mock_embed.return_value = Intent(
...
def test_intent_router():
    ...
    with patch.object(router.classifier, "classify") as mock_classify:
        ...
        mock_classify.return_value = Intent(
            type="greeting", confidence=0.9, classifier="embedding"
        )
        ...
        mock_classify.return_value = Intent(
            type="out_of_scope", confidence=0.95, classifier="llm"
        )
...
def test_convenience_function():
    ...
    with patch("src.modules.intent_classifier._classifier_instance") as mock_inst:
        mock_inst.classify.return_value = Intent(
            type="greeting", confidence=0.9, classifier="embedding"
        )
...
def test_intent_classifier_crisis_embedding_high_confidence():
    ...
    with patch.object(classifier, "_embedding_classifier") as mock_embed:
        mock_embed.return_value = Intent(
            type="crisis", confidence=0.88, classifier="embedding"
        )
...
def test_intent_classifier_crisis_embedding_low_confidence():
    ...
    with patch.object(classifier, "_embedding_classifier") as mock_embed:
        mock_embed.return_value = Intent(
            type="crisis", confidence=0.75, classifier="embedding"
        )
        mock_fallback = MagicMock()
        mock_fallback.return_value = Intent(
            type="crisis", confidence=0.98, classifier="llm"
        )
