...
class TestMentalHealthRAG(unittest.TestCase):
    ...
    def setUp(self, mock_dspy_lm, mock_inference_client, mock_hf, mock_qdrant):
        ...
        self.cache_path = os.path.join(
            tempfile.gettempdir(),
            f"test_cache_{os.getpid()}_{self._testMethodName}.pkl",
        )
    ...
    def test_load_and_preprocess_generates_documents(self, mock_read_json):
        ...
        mock_data = pd.DataFrame(
            [
                ...
                {
                    "Context": "I feel anxious.",
                    "Response": "Try to meditate.",
                },  # Duplicate context to check grouping
                {
                    "Context": "Context0",
                    "Response": "Ignore row 0",
                },  # Simulating row 0 drop logic
            ]
        )
    ...
    def test_language_alignment_prompting(self, mock_ensemble):
        ...
        mock_doc = Document(
            page_content="Mock question",
            metadata={"response": "Mock answer in English"},
        )
        ...
        self.rag.grounded_module = MagicMock(
            return_value="Respuesta simulada en Español"
        )
    ...
    def test_query_general_calls_llm(self):
        ...
        self.rag.general_module = MagicMock(
            return_value="Hello Mazen! I'm here to support you."
        )
        ...
        answer = self.rag.query_general(
            "my name is mazen", history=history, language="English"
        )
